...
class TestCaseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if is_smoke is not None and is_smoke != '':
            # 支持 true/false/1/0/True/False
            val = str(is_smoke).lower()
            if val in ('true', '1'):
                qs = qs.filter(is_smoke=True)
            elif val in ('false', '0'):
                qs = qs.filter(is_smoke=False)
    ...
    def import_cases(self, request):
        ...
        for idx, row in enumerate(reader, start=2):  # 从第 2 行开始（第 1 行是表头）
            ...
            data = {
                ...
                # 导入的用例默认设置为“停用/草稿”状态，避免直接生效
                # 即使表格里填了 status，也会被统一覆盖为 DRAFT
                'status': TestCase.Status.DRAFT,
                'is_smoke': parse_bool(row.get('is_smoke')),
